...
import PickupFormSection from '../components/PickupFormSection';
import { useLocalStorage } from '../hooks/useLocalStorage';
import { useFileUpload } from '../hooks/useFileUpload';

const ClientPickupDetails: React.FC = () => {
	...
	const [pickupVehicleNumber, setPickupVehicleNumber] = useState('');
	const [signatureName, setSignatureName] = useState('');
	const [containerCounts, setContainerCounts] = useState<Record<number, number>>({});

	// Local storage key for this client (receive version)
	const storageKey = `client-receive-details-${clientLocationId}-${facilityId}`;
	...
	useEffect(() => {
		if (location.state) {
			...
			if (savedData) {
				...
				setPickupVehicleNumber(savedData.pickupVehicleNumber || '');
			...
			navigate('/transit-plan/received/plan');
		}
	...
	const handleSubmit = async () => {
		...
		try {
			...
			console.log('📦 Calling receivedB2BInventory API...', inventoryPayload);
			const inventoryResponse = await TransitPlanApi.receivedB2BInventory(inventoryPayload);
			console.log('📦 receivedB2BInventory response:', inventoryResponse);
			...
			if (!inventoryResponse || inventoryResponse.status_code !== 200) {
				throw new Error(
					inventoryResponse?.message || 'Failed to receive inventory - no response received'
				);
			}
			...
			const initiateTransitPayload = {
				...
				vehicleNumber: pickupVehicleNumber,
			...
			setSnackbar({
				open: true,
				message: `Pickup completed successfully! DC Number: ${inventoryResponse.dc_number}`,
				type: 'success',
			});
			...
			console.error('❌ Error during pickup submission:', error);
			setSnackbar({
				open: true,
			...
			<div className='max-w-7xl mx-auto'>
				<div className='grid grid-cols-1 lg:grid-cols-5 gap-6'>
					...
					<PickupFormSection
						...
						onPickupVehicleNumberChange={setPickupVehicleNumber}
						signatureName={signatureName}
				...
				<div className='flex justify-end space-x-4 mt-6'>
					<Button
						variant='outline'
						onClick={() => navigate('/transit-plan/received/plan')}
						className='px-6 py-2'
					>
						Cancel
					</Button>
...
export default ClientPickupDetails;
